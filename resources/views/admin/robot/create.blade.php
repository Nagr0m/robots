...
@section('content')
<form class="container" action="{{route('robot.store')}}" method="post" novalidate>
	@if ($errors->count() > 0)
	<div class="row">
		<div class="col s12">
	...
	<div class='row'>
		<div class='input-field col s12'>
			<textarea class='validate materialize-textarea' name='description' id='description'>{{old('description')}}</textarea>
	...
	<div class="row">
		<div class='input-field col s12' style="display: flex; flex-flow: row wrap; justify-content: space-around;">
			...
			<div>
				<input type="checkbox" name="tags[]" value="{{$id}}" id="tag{{$id}}" {{ selected_fields($id, old('tags')) }}/>
